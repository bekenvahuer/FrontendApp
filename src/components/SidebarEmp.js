...
const SidebarEmp = ({ toggleQRView, setCalendarView }) => {
  ...
  return (
    ...
    <ul className="sidebar-menu">
      ...
      <li
        ...
        onClick={() => {
          if (location.pathname === `/${username}/calendarEmp`) {
            setCalendarView('month');
          }
        }}
      >
        <Link to={`/${username}/calendarEmp`}>
          <img src={ClickCita} alt="ClickCita" className="sidebar-icon2" />
        </Link>
      </li>
      ...
      {location.pathname === `/${username}/calendarEmp` && (
