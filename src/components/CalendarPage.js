...
const CalendarPage = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`${API_BASE_URL}/events`)
            .then(response => {
                // Filtrar eventos que contengan el username en el título
                const filteredEvents = response.data.filter(event =>
                    event.title.includes(`/${username}`)
                );

                // Formatear los eventos filtrados
                ...
                // Actualizar el estado con los eventos filtrados y formateados
    ...
    const handleSelectSlot = (slotInfo) => {
        ...
        if (hasEvents) {
            ...
            navigate(`/${username}/events/create`, { state: { start: slotInfo.start, end: slotInfo.end } });
        }
    };
    ...
    //para mostrar los datos de localstorage
    (() => {
        const data = {};
    ...
    const updateEventAndCreateSale = async (id) => {
        try {
            await axios.post(`${API_BASE_URL}/events/${id}`);
    ...
    return (
        <div className='Container-calendar'>
            <Sidebar toggleQRView={toggleQRView} setCalendarView={setCalendarView} />
            <Carrucel />

            <div className='Title-calendar-cont'>
                <h1 className='Title-calendar'>Tu agenda {formattedName}!</h1>
            </div>
            <div>
                {/* Modal para teléfono y cumpleaños */}
                <Modal
                                ...
                                <div className="modal-buttons">
                                    <button
                                        onClick={() => {
                                            ...
                                            setIsActionModalOpen(false);
                                            navigate(`/${username}/events/create`, { state: { start: selectedSlot.start, end: selectedSlot.end } });
                                        }}
                            ...
                            <>
                                {selectedEvent ? (
                                    <>
                                        ...
                                        <p><strong>Cliente:</strong> {selectedEvent.title.split('/').slice(2).join(' ').replace('Client:', '').trim()}</p>
                                        ...
                                        <div className="modal-buttons">
                                            <Link
                                                to={`/${username}/events/edit/${selectedEvent.id}`}
                                                className="btn btn-primary"
                                                style={{ backgroundColor: '#bbccf9', borderColor: '#bbccf9', color: '#090027' }}
                                            >
                                                <FaEdit />
                                            </Link>
                                            ...
                                            <button
                                                ...
                                                style={{ backgroundColor: '#7596f0', borderColor: '#7596f0', color: '#090027' }}
                                            >
                                                <FaTrash />
                                            </button>
                                        </div>
                    ...
                    <div className='Container-qr'>
                        ...
                        <div className="qr-layout-container">
                            ...
                            <div className="qr-logo-section">
                                ...
                                />
                                <div className="qr-plan-info">
                                    <div className="logo-info-value">Plan</div>
                                    <div className="logo-info-label">{businessData.planDePago}</div>
                                    {/* <div className="logo-info-value">hasta</div>
                                    <div className="logo-info-label">☞ colocar aqui:01/06/2025</div> */}
                                </div>
                            </div>
